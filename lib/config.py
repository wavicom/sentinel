...
sentinel_cfg = WaviConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_wavid_proto_version_with_sentinel_ping = 70207


def get_wavi_conf():
    home = os.environ.get('HOME')

    wavi_conf = os.path.join(home, ".wavicore/wavi.conf")
    if sys.platform == 'darwin':
        wavi_conf = os.path.join(home, "Library/Application Support/WaviCore/wavi.conf")

    wavi_conf = sentinel_cfg.get('wavi_conf', wavi_conf)
