...
def is_valid_wavi_address(address, network='mainnet'):
    ...
    wavi_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Wavi address
        return False

    if (address_version != wavi_version):
        return False
...
# shims 'til we can fix the wavid side
def SHIM_serialise_for_wavid(sentinel_hex):
    from models import WAVID_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = WAVID_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in wavid
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # wavid expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    wavid_hex = serialise(obj)
    return wavid_hex


# shims 'til we can fix the wavid side
def SHIM_deserialise_from_wavid(wavid_hex):
    from models import WAVID_GOVOBJ_TYPES

    # unpack
    obj = deserialise(wavid_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('wavi.conf')
