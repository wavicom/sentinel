...
class GovernanceObject(BaseModel):
    ...
    # sync wavid gobject list with our local relational DB backend
    @classmethod
    def sync(self, wavid):
        golist = wavid.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_wavid(wavid, item)
    ...
    @classmethod
    def import_gobject_from_wavid(self, wavid, rec):
        import decimal
        import wavilib
        import inflection
        ...
        # shim/wavid conversion
        object_hex = wavilib.SHIM_deserialise_from_wavid(object_hex)
        objects = wavilib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from wavid...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from wavid, with every run
        ...
        # get/create, then sync payment amounts, etc. from wavid - Wavid is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(wavid)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from wavid! %s" % e)
            govobj.vote_delete(wavid)
            return (govobj, None)
    ...
    def vote_delete(self, wavid):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(wavid, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, wavid, signal, outcome):
        import wavilib
        ...
        output = wavid.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = wavilib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(wavid, signal)

    def sync_network_vote(self, wavid, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = wavid.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = WAVID_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 wavi addr, non-multisig
            if not wavilib.is_valid_wavi_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Wavi address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (wavilib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/WaviDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import wavilib
        obj_data = wavilib.SHIM_serialise_for_wavid(self.serialise())
    ...
    def prepare(self, wavid):
        try:
            object_hash = wavid.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = WAVID_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not wavilib.is_valid_wavi_address(addr, config.network):
    ...
    def hash(self):
        import wavilib
        return wavilib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = WAVID_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, wavid):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - wavid.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, wavid):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - wavid.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, wavid):
        now = int(time.time())
        return (self.created_at < (now - wavid.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, wavid):
        if self.is_expired(wavid):
            return False

        return True

    def is_deletable(self, wavid):
        if self.is_expired(wavid):
            return True
