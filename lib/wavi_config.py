...
class WaviConfig():
    ...
    def slurp_config_file(self, filename):
        # read wavi.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is wavi.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        ...
        # convert to an int if taken from wavi.conf
