...
class WaviDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some wavid methods
    ...
    @classmethod
    def from_wavi_conf(self, wavi_dot_conf):
        from wavi_config import WaviConfig
        config_text = WaviConfig.slurp_config_file(wavi_dot_conf)
        creds = WaviConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = wavilib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = wavilib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = wavilib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_wavid_proto_version_with_sentinel_ping)
