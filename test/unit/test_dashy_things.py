...
def test_valid_wavi_address():
    from wavilib import is_valid_wavi_address

    main = valid_wavi_address()
    test = valid_wavi_address('testnet')

    assert is_valid_wavi_address(main) is True
    assert is_valid_wavi_address(main, 'mainnet') is True
    assert is_valid_wavi_address(main, 'testnet') is False

    assert is_valid_wavi_address(test) is False
    assert is_valid_wavi_address(test, 'mainnet') is False
    assert is_valid_wavi_address(test, 'testnet') is True


def test_invalid_wavi_address():
    from wavilib import is_valid_wavi_address

    main = invalid_wavi_address()
    test = invalid_wavi_address('testnet')

    assert is_valid_wavi_address(main) is False
    assert is_valid_wavi_address(main, 'mainnet') is False
    assert is_valid_wavi_address(main, 'testnet') is False

    assert is_valid_wavi_address(test) is False
    assert is_valid_wavi_address(test, 'mainnet') is False
    assert is_valid_wavi_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from wavilib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % wavilib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(wavilib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(wavilib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(wavilib.blocks_to_seconds(16616)) == 2612035
